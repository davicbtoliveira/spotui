...
const (
	...
	// ApiRequestTypeNativeCatalog exposes authenticated catalog reads through
	// the same native spclient used by playback. It deliberately does not use
	// the public Web API, whose application quota is shared by unofficial
	// clients.
	ApiRequestTypeNativeCatalog       ApiRequestType = "native_catalog"
	...
	ApiRequestTypeSetAutoplay         ApiRequestType = "autoplay"
...
const (
	...
	ApiEventTypeAccountProduct ApiEventType = "account_product"
)
...
// ApiRequestDataNativeCatalog describes a read against Spotify's native
// authenticated catalog protocols. Kind is one of liked, playlists,
// saved_albums, playlist, album, artist, top_artists, top_tracks, or search.
type ApiRequestDataNativeCatalog struct {
	Kind   string
	URI    string
	Query  string
	Offset int
...
// ApiResponseNativeCatalog contains a JSON payload produced by the daemon's
// native catalog adapter. Keeping the payload opaque at this boundary avoids
// leaking protocol protobufs into applications embedding go-librespot.
type ApiResponseNativeCatalog struct {
	Payload []byte
}

type ApiResponseSearch struct {
	Tracks []ApiResponseSearchTrack
	Total  int
	Offset int
}

type ApiResponseSearchTrack struct {
	Uri         string
	Name        string
	ArtistNames []string
	AlbumName   string
...
type ApiEventDataAccountProduct struct {
