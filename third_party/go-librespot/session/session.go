...
func NewSessionFromOptions(ctx context.Context, opts *Options) (*Session, error) {
	...
	case InteractiveCredentials:
		...
		serverCancel()
		if result.Err != nil {
			return nil, fmt.Errorf("oauth2 callback: %w", result.Err)
		}
...
func notifyAuthURL(hook func(string), url string) {
	if hook != nil {
		hook(url)
	}
}
