...
func (p *PlaybackStream) handleEvents(events chan struct{}) {
	...
	for range events {
		...
		if err != nil {
			...
			// An event query can time out while the server is busy. This is
			// transient and must not terminate the hosting application; a later
			// event will retry the query and report the current volume.
			continue
		}
...
func consumeStarted(started <-chan bool) {
	select {
	case <-started:
	default:
	}
...
func (p *PlaybackStream) notifyStarted() {
	if !p.state.is(running) {
		return
	}
	select {
...
func (p *PlaybackStream) prepareResume() bool {
